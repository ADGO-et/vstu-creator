  ...
  const handleInsert = async () => {
    ...
    for (let i = 0; i < questions.length; i++) {
        try {
            ...
            setInsertStatus(error instanceof Error ? error.message : "Failed to insert questions. Please try again.");
    ...
    <div>
      <div className="">
        ...
        <h1 className='text-lg font-bold text-primary mb-5'>
          This section allows you to generate a question based on the text you previously extracted.
        </h1>
        <div className="flex flex-col space-y-4">
          <div className="flex items-center gap-4">
            <label htmlFor="" className='text-gray-600 text-sm'>Number of questions</label>
            <input
            type="number"
          ...
          <div className="w-[150px] flex items-center gap-3">
            <label htmlFor="" className='text-gray-600 text-sm'>Difficulty</label>
          ...
          <div className="bg-[#e2ffde] p-2 rounded-md w-fit px-4">
            <span className='text-gray-500 text-xs'>the difficulty level ranges from 1 - 10. 1 means very easy questions and 10 means very hard questions.</span>
          </div>
          ...
          >
            {!generateQuestion.isPending ? 'Generate Questions' : <Spinner spin={true} property={''}/>}
          </Button>
          ...
          { questions.length > 0 && (
            <div className="mt-4 space-y-4 overflow-auto max-h-[500px]">
              {questions.map((q, idx) => (
                <div key={idx} className="p-4 border rounded-md shadow bg-[#e2ffde]">
                  <p className="font-semibold">{q.question}</p>
                  <ul className="list-disc pl-5">
                    {q.choices.map((option, i) => (
                      <li key={i} className='text-gray-600 text-sm'>{option}</li>
                    ))}
                  </ul>
                </div>
              ))}
          ...
          )}
          {
            questions.length === 0 && generateQuestion.isSuccess && (
              <p className='text-primary'>No questions generated. Try increasing the limit.</p>)
          }
          {
            questions.length === 0 && generateQuestion.isError && (
              <p className='text-red-600'>Failed to generate questions. Please try again.</p>)
          }
          {
            questions.length > 0 && (
                <div className="flex items gap-8 py-10">
                    <Button className='w-fit bg-destructive' onClick={() => handleRetry()}>Retry</Button>
                    <Button 
                        className='w-fit' 
                        onClick={handleInsert}
                        disabled={isInserting}
                    >
                      {isInserting ? `Inserting... (${progress}%)` : "Insert the Questions Now"}
                    </Button>
                </div>
            )
          }
          {insertStatus && (
              <p className={insertStatus.includes("successfully") ? 'text-primary' : 'text-red-600'}>
                  {insertStatus}
              </p>
          )}
        </div>
